...
session_start();
global $mgv , $tgv, $ngs, $ph, $sal, $dchi, $dt, $qq ,$p, $r, $ml;
$mgv = $tgv = $ngs = $ph = $sal = $dchi = $dt = $qq = $p = $r = $ml = "";
 if(isset($_POST['adduser']))
 {
  if($_POST['MaGV'] == NULL){
    echo "vui lòng nhập mã giáo viên";
  }else{
    $mgv = $_POST['MaGV'];
  }
  ...
  if($mgv && $tgv && $ml && $ngs && $ph && $sal && $dchi && $dt && $qq && $p && $r)
  {
   ...
   {
    echo "giáo viên này đã tồn tại rồi<br />";
   }
   else
   {
    $sql2="insert into giaovien(MaGV, TenGV, MaLop, NgaySinh, Phai, Luong, DiaChi, DienThoai, QueQuan, Password) 
    values ('".$mgv."','".$tgv."','".$ml."','".$ngs."','".$ph."','".$sal."','".$dchi."','".$dt."','".$qq."','".$p."')";
    $query2=mysqli_query($connection,$sql2);
    echo "Đã thêm giáo viên mới thành công";
   }
  }
...
Mã giáo viên:   <input type='text' name='MaGV' size='25' /><br />
...
<input type='submit' name='adduser' value='Thêm giáo viên' />
</form>
